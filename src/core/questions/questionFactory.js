...
// ReflectQuestion only renders questionText + options (no visual aid), so
// every question type whose answer depends on a visual aid (ten-frame,
// base-10 blocks, or a counted picture) is excluded here — only the
// types fully answerable from text alone (visual === 'none') are used.
export function getReflectQuestions(count = 5) {
  const session = getSession().filter((q) => q.visual === 'none');
