 ...
 *  - onDismiss: () => void (manual dismiss — required for wrong answers, or
 *    called automatically when autoAdvanceMs is set)
 *  - autoAdvanceMs: number (if set, auto-calls onDismiss after delay,
 *    regardless of correct/wrong — used for read-only/timed flows with no retry)
 */
export default function FeedbackOverlay({
  ...
  React.useEffect(() => {
    if (visible && autoAdvanceMs && onDismiss) {
      const t = setTimeout(onDismiss, autoAdvanceMs);
      return () => clearTimeout(t);
    }
  }, [visible, autoAdvanceMs, onDismiss]);
      ...
      {visible && (
        ...
        >
          ...
          >
            ...
            {!correct && onDismiss && !autoAdvanceMs && (
